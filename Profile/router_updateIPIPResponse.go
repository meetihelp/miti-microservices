...
func UpdateIPIPResponse(w http.ResponseWriter, r *http.Request){
	...
	ipipStatus:=UpdateIPIPResponseDB(userId,data)
	auth.UpdateIPIPStatus(userId,ipipStatus)
	UpdateIPIPScore(userId)
	util.Message(w,200)
}

func getDataInQuestionResponseForm(questionResponse QuestionResponse,data map[string]int) (int,QuestionResponse){
	ipipStatus:=0
	...
	return ipipStatus,questionResponse
}